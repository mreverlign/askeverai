...
class EnhancedRAGReActAgent:
    ...
    def __init__(self, llm_client: LLMClient, db_tools: DatabaseTools, embedder: FocusedColumnEmbedder):
    ...
    def _extract_relevant_context(self, query: str, top_k: int = 10) -> Dict[str, Any]:
        ...
        detected_tables = self._detect_table_names(query, metadata_results + datamodel_results)
        ...
        logger.info(f"🔗 Relationships: Found {len(all_relationship_results)} unique matches")
    ...
    def _get_examples_from_metadata(self, context: Dict[str, Any]) -> Dict[str, List[str]]:
        """
    ...
    def _format_rag_context(self, context: Dict[str, Any], include_schema: bool = True) -> str:
        ...
        parts.append(f"- Semantic Weight: {self.embedder.semantic_weight} (conceptual similarity)")
        parts.append("")
        ...
        if include_schema and context.get('relevant_tables'):
            ...
            parts.append("**IMPORTANT: Use these EXACT column names in your SQL queries**")
            parts.append("")
                ...
                if schema_result['success']:
                    for table_name, schema_info in schema_result['schemas'].items():
                        ...
                        if schema_info.get('primary_keys'):
                            parts.append(f"Primary Keys: {', '.join(f'`{pk}`' for pk in schema_info['primary_keys'])}")
                    ...
                    logger.warning(f"Could not load schema: {schema_result.get('error')}")
                    parts.append(f"⚠️ Schema loading skipped (will use sql_db_schema action instead)")
                    parts.append("")
        ...
        if context['relationships']:
            for i, rel in enumerate(context['relationships'][:8], 1):
                ...
                if rel['join_condition']:
                    ...
                    parts.append(f"   **SQL:** `JOIN {rel['foreign_table']} ON {rel['join_condition']}`")
                
                parts.append(f"   Relationship Type: {rel['relationship_type']}")
                parts.append(f"   Scores: Hybrid={rel['hybrid_score']:.3f}, BM25={rel['bm25_score']:.3f}, Semantic={rel['semantic_score']:.3f}")
                parts.append("")
        else:
        ...
        if context['metadata_results']:
            for i, result in enumerate(context['metadata_results'][:8], 1):
                ...
                if example and str(example).strip() and str(example) not in ['nan', 'none', '']:
                    parts.append(f"   **Example Values: {example}**")
                    parts.append(f"   💡 TIP: Use these exact values in WHERE clauses (e.g., WHERE {column} = 'value' or WHERE {column} IN ('val1', 'val2'))")

                parts.append(f"   Scores: Hybrid={result.get('hybrid_score', 0):.3f}, BM25={result.get('bm25_score', 0):.3f}, Semantic={result.get('semantic_score', 0):.3f}")
                parts.append("")
        else:
        ...
        if examples_by_column:
            parts.append("### 💎 AVAILABLE EXAMPLE VALUES (Use EXACT Case - ILIKE/LIKE PROHIBITED)")
            parts.append("**CRITICAL: Use these EXACT values with proper casing. NEVER use ILIKE or LIKE!**")
            parts.append("")
            for column, example_values in sorted(examples_by_column.items())[:10]:
                if example_values:
                    # Format values as SQL IN clause
                    formatted_values = "', '".join(example_values[:8])  # Show up to 8 examples
                    parts.append(f"**Column: `{column}`**")
                    parts.append(f"   Available Values: {', '.join(example_values[:8])}")
                    parts.append(f"   SQL Example: `WHERE {column} = '{example_values[0]}'`")
                    if len(example_values) > 1:
                        parts.append(f"   SQL Multiple: `WHERE {column} IN ('{formatted_values}')`")
                    parts.append("")
            parts.append("🚫 **ILIKE and LIKE are FORBIDDEN - Use exact values with = or IN operators only**")
            parts.append("")
        ...
        if context['datamodel_results']:
            for i, result in enumerate(context['datamodel_results'][:5], 1):
                parts.append(f"**Data Model {i}:** (score: {result.get('hybrid_score', 0):.3f})")
                
                # Show table names from data model
                for key, value in result.items():
                    if key not in ['source', 'row_id', 'score', 'distance', 'text', 
        ...
        parts.append("1. **ALWAYS check '💎 AVAILABLE EXAMPLE VALUES' section** - Use EXACT casing, ILIKE/LIKE FORBIDDEN!")
        parts.append("2. **ALWAYS use the exact join conditions provided above**")
        parts.append("3. **Use fully qualified column names** (e.g., `dim_client.clientid`, not just `clientid`)")
        parts.append("4. **Reference the relationship section** when joining tables")
        parts.append("5. **GROUP BY is MANDATORY** when using aggregates (SUM, COUNT, AVG) with non-aggregate columns")
        parts.append("6. **Verify table names** from the relevant tables summary")
        parts.append("")
        
        return "\n".join(parts)

    def _create_react_prompt(self, user_query: str, rag_context: str,
                            iteration: int, previous_steps: List[Dict]) -> str:
...
**Example:**
```
Thought: User wants to see top clients AND top products separately, so I need two queries.
Action: sql_db_query[SELECT c.clientname, SUM(f.amount) as total FROM fact_transaction_detail f JOIN dim_client c ON f.clientid = c.clientid WHERE f.amount > 0 GROUP BY c.clientname ORDER BY total DESC LIMIT 10]
sql_db_query[SELECT i.productname, SUM(f.amount) as total FROM fact_transaction_detail f JOIN dim_item i ON f.itemid = i.skuid WHERE f.amount > 0 GROUP BY i.productname ORDER BY total DESC LIMIT 10]
```
...
3. **Date Range (use appropriate range):**
   ```sql
   WHERE f.dateid BETWEEN '20230101' AND '20251231'
   ```
...
**Product Dimensions:**
- **dim_item** (connects via `fact.itemid` ** `skuid`)
  - Product name|details: `skuid`
  - Product discription: `productname`
...
"""
        ...
        try:
            ...
            if rag_context_data['relationships']:
                ...
                for rel in rag_context_data['relationships'][:5]:
                    logger.info(f"   {rel['primary_table']}.{rel['primary_key']} = {rel['foreign_table']}.{rel['foreign_key']}")
                logger.info("")
            ...
            for iteration in range(1, self.max_iterations + 1):
                ...
                response = self.llm_client.generate(prompt, temperature=0.0, max_tokens=4000)
                ...
                if action:
                    ...
                    thought_action_log.append({"type": "observation", "content": observation})
                    previous_steps.append({"type": "observation", "content": observation})

                    # Check if we have SQL queries
                    if "sql_db_query[" in action.lower() and "row" in observation.lower():
                        ...
                        if "Executed" in observation and "quer" in observation:
                            ...
                            for block in query_blocks[1:]:  # Skip first block (header)
                                if "Query" in block and "of" in block:
                                    ...
                                    if rows_match:
                                        query_result['row_count'] = int(rows_match.group(1))
            ...
            result = {
                ...
                "sql": final_sql,
                "all_sql_queries": all_sql_queries if all_sql_queries else ([final_sql] if final_sql else []),
    ...
    def _extract_multiple_sql_queries(self, action: str) -> List[str]:
        ...
        if queries:
            logger.info(f"✅ Extracted {len(queries)} queries from sql_db_query[] blocks")
            return queries
        ...
        if sql:
            ...
            if ';' in sql:
                ...
                for query in sql.split(';'):
                    query = query.strip()
                    if query and query.upper().startswith(('SELECT', 'WITH', 'INSERT', 'UPDATE', 'DELETE')):
        ...
        try:
                ...
                if tables:
                    ...
                    if result['success']:
                        ...
                        for table_name, schema_info in result['schemas'].items():
                            ...
                            for col in schema_info['columns']:
                                schema_output.append(f"  - {col['name']} ({col['type']})")

                            if schema_info.get('primary_keys'):
                                schema_output.append(f"Primary Keys: {', '.join(schema_info['primary_keys'])}")

                            if schema_info.get('foreign_keys'):
                                schema_output.append("Foreign Keys:")
                                for fk in schema_info['foreign_keys']:
                                    schema_output.append(f"  - {fk['column']} → {fk['references_table']}.{fk['references_column']}")

                            schema_output.append(f"Row Count: {schema_info['row_count']:,}")
            ...
            if "sql_db_query_checker" in action_lower:
                ...
                if sql:
                    ...
                    if result['success']:
                        return f"Query validation: Valid - {result.get('message', 'OK')}"
                    else:
                        return f"Query validation: Invalid - {result.get('error', 'Unknown error')}"

            # Check for SQL query execution (both explicit and implicit)
            if "sql_db_query" in action_lower or ("```sql" in action_lower) or ("select " in action_lower and "from " in action_lower):
                ...
                for idx, sql in enumerate(sql_queries, 1):
                    ...
                    if validation['warnings']:
                        logger.warning(f"SQL validation warnings: {validation['warnings']}")
                ...
                response_parts = []
                response_parts.append(f"Executed {len(sql_queries)} quer{'y' if len(sql_queries) == 1 else 'ies'}")
                response_parts.append(f"Total execution time: {total_execution_time:.2f}s\n")

                for result in all_results:
                    response_parts.append(f"\n{'='*60}")
                    response_parts.append(f"Query {result['query_number']} of {len(sql_queries)}")
                    response_parts.append(f"{'='*60}")

                    if result['success']:
                        response_parts.append(f"✅ Success | Rows: {result.get('row_count', 0)} | Time: {result.get('execution_time', 0):.2f}s")
                        if result.get('preview'):
                            response_parts.append(f"\nPreview:\n{result['preview']}")
                        elif result.get('message'):
                            response_parts.append(f"\n{result['message']}")
                    else:
                        response_parts.append(f"❌ Failed: {result.get('error', 'Unknown error')}")
    ...
    def _extract_sql_from_action(self, action: str) -> Optional[str]:
        ...
        if not sql:
            match = re.search(r'sql_db_query_checker\[(.+)\]', action, re.IGNORECASE | re.DOTALL)
            if match:
                sql = match.group(1).strip()
        ...
        if not sql:
            ...
            match = re.search(r'\b(WITH\s+.+?\bSELECT\b.+?)(?:;|\]|$)', action, re.IGNORECASE | re.DOTALL)
            if match:
                sql = match.group(1).strip()

        if not sql:
            # SELECT statement with all possible clauses
            match = re.search(r'\b(SELECT\s+.+?)(?:;|\]|```|$)', action, re.IGNORECASE | re.DOTALL)
            if match:
                sql = match.group(1).strip()

        if not sql:
            # INSERT statement
            match = re.search(r'\b(INSERT\s+INTO\s+.+?)(?:;|\]|```|$)', action, re.IGNORECASE | re.DOTALL)
            if match:
                sql = match.group(1).strip()

        if not sql:
            # UPDATE statement
            match = re.search(r'\b(UPDATE\s+.+?\s+SET\s+.+?)(?:;|\]|```|$)', action, re.IGNORECASE | re.DOTALL)
            if match:
                sql = match.group(1).strip()

        if not sql:
            # DELETE statement
            match = re.search(r'\b(DELETE\s+FROM\s+.+?)(?:;|\]|```|$)', action, re.IGNORECASE | re.DOTALL)
            if match:
                sql = match.group(1).strip()
    ...
    def _validate_sql_clauses(self, sql: str) -> Dict[str, Any]:
        ...
        try:
            ...
            has_join = any(result['clauses'].get(j) for j in ['JOIN', 'LEFT_JOIN', 'RIGHT_JOIN', 'FULL_JOIN'])
            if has_join and not result['clauses'].get('ON') and not result['clauses'].get('CROSS_JOIN'):
                result['errors'].append("JOIN clause without ON condition")
                result['valid'] = False

            # Check for GROUP BY when using aggregates without window functions
            has_aggregate = any(result['clauses'].get(a) for a in ['SUM', 'COUNT', 'AVG', 'MIN', 'MAX'])
            has_window = result['clauses'].get('OVER')

            if has_aggregate and not has_window and not result['clauses'].get('GROUP_BY'):
                # Extract SELECT clause to analyze columns
                select_match = re.search(r'SELECT\s+(DISTINCT\s+)?(.*?)\s+FROM', sql_upper, re.DOTALL | re.IGNORECASE)
                if select_match:
                    ...
                    cleaned_select = re.sub(r'\b(SUM|COUNT|AVG|MIN|MAX|STRING_AGG|ARRAY_AGG)\s*\([^)]+\)', '', select_part, flags=re.IGNORECASE)
                    cleaned_select = re.sub(r'\bDISTINCT\s+', '', cleaned_select, flags=re.IGNORECASE)
                    cleaned_select = re.sub(r'\s+AS\s+\w+', '', cleaned_select, flags=re.IGNORECASE)  # Remove aliases
                    ...
                    has_non_aggregate_columns = bool(cleaned_select and cleaned_select not in [',', '', '*'])
            ...
            if result['clauses'].get('HAVING') and not result['clauses'].get('GROUP_BY'):
            ...
            if result['clauses'].get('ORDER_BY') and sql_upper.count('SELECT') > 1:
                result['warnings'].append("ORDER BY in subquery may be ignored unless used with LIMIT")

            # Check for TOP (SQL Server syntax - invalid in PostgreSQL)
            if re.search(r'\bTOP\s+\d+', sql_upper):
                result['errors'].append("TOP is SQL Server syntax. Use LIMIT for PostgreSQL")
                result['valid'] = False
