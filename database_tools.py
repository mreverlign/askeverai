...
class SchemaContextManager:
    ...
    def get_relevant_tables(self, user_question: str) -> List[str]:
        ...
        keyword_map = {
            ...
            'rate|unit price|item price|price per': 
                ['fact_transaction_detail'],
            
            # Product related
            'product|item|sku|inventory|goods|merchandise': 
                ['dim_item', 'dim_item_category', 'dim_subcategory_item', 'fact_transaction_detail'],
            
            'category|product category|product line': 
                ['dim_item_category', 'dim_subcategory_item'],
            
            'subcategory|product subcategory|subline': 
            ...
            'configuration|option|customization|variant': 
                ['dim_item'],
            
            'brand|manufacturer|supplier|vendor': 
                ['dim_item', 'dim_brand_partner_item'],
            
            # Customer & Client
            'client|customer|buyer|purchaser|account': 
                ['dim_client', 'fact_transaction_detail'],
            
            'client category|customer type|account type|client classification': 
                ['dim_client_category'],
            
            'key account|major account|strategic account': 
                ['dim_client', 'dim_partner'],
            
            # Date & Time
            'date|time|month|year|quarter|week|when|period|fiscal|timeline': 
                ['dim_date', 'fact_transaction_detail'],
            
            'close date|closed|closing|completion': 
                ['fact_transaction_detail'],
            
            'created date|creation|start date': 
                ['fact_transaction_detail'],
            
            'ship date|shipping date|delivery date|shipment date': 
                ['fact_transaction_detail'],
            
            'expected close|expected date|forecast date': 
                ['fact_transaction_detail'],
            
            # Location
            'city|location|urban|metro|geography': 
                ['fact_transaction_detail', 'dim_partner', 'dim_specifier1'],
            
            'state|province|region': 
                ['fact_transaction_detail', 'dim_partner', 'dim_specifier1'],
            
            'country|nation|international': 
                ['fact_transaction_detail', 'dim_partner'],
            
            'zip|postal|zipcode|postal code': 
                ['fact_transaction_detail'],
            
            'shipping|delivery|shipment|freight|delivered|shipped': 
                ['fact_transaction_detail', 'dim_transaction_type'],
            ...
            'sales rep|representative|salesperson|sales team|seller': 
                ['dim_sales_rep', 'fact_transaction_detail'],
            
            # Opportunity & Deal
            'opportunity|opp|deal|potential sale|pipeline': 
                ['fact_transaction_detail'],
            
            'probability|likelihood|chance|win rate': 
                ['fact_transaction_detail'],
            ...
            'project|job|engagement|initiative': 
                ['dim_project', 'dim_project_type', 'fact_transaction_detail'],
            
            'project manager|pm|project lead': 
                ['dim_project_manager', 'fact_transaction_detail'],
            
            'project type|project category|job type': 
                ['dim_project_type'],
            
            # Partners & Relationships
            'partner|business partner|affiliate': 
                ['dim_partner', 'fact_transaction_detail'],
            
            'dealer|distributor|reseller|channel': 
                ['dim_dealer_alignment'],
            
            'design firm|architect|designer|design company': 
                ['dim_design_firm'],
            
            # Specifiers
            'specifier|specification|spec': 
                ['dim_specifier1', 'dim_specifier2', 'dim_specifier_type', 'dim_specifier_sub_type'],
            
            'specifier type|spec type': 
                ['dim_specifier_type'],
            
            'specifier subtype|spec subtype': 
                ['dim_specifier_sub_type'],
            
            # Market & Industry
            'market|vertical|industry|sector|segment': 
                ['dim_vertical_market'],
            
            'vertical market|industry vertical': 
                ['dim_vertical_market'],
            
            # Transaction Type & Status
            'transaction type|order type|doc type|document type': 
                ['dim_transaction_type', 'fact_transaction_detail'],
            ...
            'status|state|condition|transaction status': 
                ['dim_transaction_status', 'fact_transaction_detail'],
            
            # End User
            'end user|final user|consumer|ultimate customer': 
                ['dim_end_user', '_type', 'fact_transaction_detail'],
            
            'end user type|consumer type|user category': 
                ['dim_end_user_type'],
            
            # Other attributes
            'memo|note|comment|remark|description': 
                ['fact_transaction_detail'],
            ...
            if schema.get('numeric_stats'):
                ...
                for col_name, stats in schema['numeric_stats'].items():
                    if stats['min'] is not None:
                        avg_formatted = f"{stats['avg']:.2f}" if stats['avg'] is not None else 'N/A'
                ...
                try:
                    with self.conn.cursor() as cur:
                        ...
                        try:
                            ...
                            logger.warning(f"Could not get primary keys for {table}: {pk_error}")
                            primary_keys = []
                        ...
                        try:
                            ...
                            logger.warning(f"Could not get row count for {table}: {count_error}")
                            row_count = 0
                        ...
                        try:
                            ...
                            logger.warning(f"Could not get sample rows for {table}: {sample_error}")

                        # Get statistics for numeric columns
                        numeric_stats = {}
                        for col in columns:
                            if col['type'].lower() in ['integer', 'bigint', 'numeric', 'decimal', 'real', 'double precision']:
                                try:
                                    cur.execute(f'SELECT MIN("{col["name"]}"), MAX("{col["name"]}"), AVG("{col["name"]}") FROM "{table}" WHERE "{col["name"]}" IS NOT NULL')
                                    result = cur.fetchone()
                                    ...
                                    logger.debug(f"Could not get stats for {table}.{col['name']}: {stat_error}")
                    ...
                    logger.error(f"Error getting schema for table {table}: {table_error}")
        ...
        try:
            ...
            with self.conn.cursor() as cur:
                try:
                    ...
                    self.conn.rollback()
                    self.transaction_in_error = False  # Reset error state after rollback
                    return {
                        "success": False,
                        "error": str(e),
