...
class FocusedColumnEmbedder:
    def __init__(
        ...
        logger.info(
            f"🔄 Hybrid weights - BM25: {bm25_weight}, Semantic: {semantic_weight}"
        )
    ...
    def _create_focused_metadata_text(
        self,
        table_name: str,
        column_name: str,
        data_type: str,
        ...
        if description and description not in ["nan", "none", ""]:
            ...
            key_words = [w for w in desc_words if len(w) > 4][
                :5
    ...
    def _create_focused_relationship_text(
        self,
        primary_table: str,
        primary_key: str,
        foreign_table: str,
    ...
    def _clean_relationship_row(
        self, row: pd.Series, col_mapping: Dict[str, str]
    ) -> Dict[str, str]:
    ...
    def _hybrid_search(
        ...
        semantic_score_map = {
            idx: score for idx, score in zip(indices[0], semantic_scores)
        }
    ...
    def save_indices(self, save_dir: str):
        ...
        if self.relationships_index:
            faiss.write_index(
                self.relationships_index, str(save_path / "relationships.index")
            )
    ...
    def load_indices(self, load_dir: str):
        ...
        if (load_path / "relationships.index").exists():
            self.relationships_index = faiss.read_index(
